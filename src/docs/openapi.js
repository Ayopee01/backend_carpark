...
const openapi = {
  ...
  tags: [
    ...
    { name: 'Admin Payment Gateway', description: 'Admin Omise payment endpoints. Requires transactions permission.' },
    ...
    schemas: {
      ...
      AdminOmiseChargeRequest: {
        ...
        properties: {
          ...
          plateNo: { type: 'string', example: '3à¸‡à¸ˆ9012', description: 'Fallback lookup key when transactionId is not supplied.' },
          source: { type: 'string', example: 'src_test_123', description: 'Omise source id created by the Admin Frontend with Omise.js.' },
          token: { type: 'string', example: 'tokn_test_123', description: 'Omise card token created by the Admin Frontend with Omise.js.' },
          sourceType: { type: 'string', example: 'promptpay' },
          ...
          returnUri: { type: 'string', example: 'https://carpark-uat.biza.me/admin/payment/result' },
        },
      },
      OmiseChargeResponse: {
        type: 'object',
      ...
      PrinterProvisionRequest: {
        type: 'object',
        required: ['deviceName'],
        properties: {
          deviceName: { type: 'string', example: 'Printer Gate A' },
          name: { type: 'string', example: 'Printer Gate A' },
          deviceCode: { type: 'string', example: 'PRN-GATE-A' },
          deviceId: { type: 'string', example: 'PRN-GATE-A' },
          location: { type: 'string', example: 'Gate A' },
          printerRole: { type: 'string', example: 'receipt' },
          connectionType: { type: 'string', example: 'lan' },
          ipAddress: { type: 'string', example: '192.168.1.80' },
          note: { type: 'string', example: 'Printer for kiosk and barrier gate' },
        },
      },
  ...
  paths: {
    ...
    '/api/v1/devices': {
      get: {
        ...
        parameters: [
          query('deviceType', { type: 'string', enum: ['kiosk', 'barrier_gate', 'camera', 'printer'] }, 'kiosk'),
    ...
    '/api/v1/devices/printers/provision': {
      post: {
        ...
        responses: {
          201: ok('Printer provisioned', ref('PrinterProvisionResponse')),
          400: error('deviceName is required'),
          409: error('Device code already exists'),
          ...bearer403,
        },
    ...
    '/api/v1/client/events': {
      get: {
        ...
        description: 'Shared event stream for kiosk, barrier gate, and public/mobile clients. No admin Bearer token is required. If deviceId is supplied, valid device credentials are required and the device may be kiosk or barrier_gate. For kiosk/barrier clients, the open SSE connection refreshes the device heartbeat on each server ping so the device remains online while connected. LPR results are emitted as lpr_detected after POST /api/v1/transactions is processed. Use gateId, direction, and optionally cameraId to subscribe a barrier-gate screen to only its mapped camera events. lpr_detected includes targetDeviceId/deviceId from the matched Barrier Gate mapping when available, but stream filtering is based on gateId/direction/cameraId. Barrier Gate screens should use lpr_detected.action and lpr_detected.paymentRequired: OPEN_GATE means allow/open, PAYMENT_REQUIRED means do not open and show or route to payment using remainingAmount/netAmount/totalPaid. Entry transactions use status pending. lpr_detected always includes checkedAt, and amount fields default to 0 when no payment is required. Current events include connected, ping, theme_updated, and lpr_detected.',
        security: [...deviceAuth, {}],
        parameters: [
    ...
    '/api/v1/admin/payment/omise/charge': {
      post: {
        ...
        responses: {
          201: ok('Admin Omise charge created', ref('OmiseChargeResponse')),
          400: error('transactionId/plateNo, source/token, channel, or amount is invalid'),
          404: error('Transaction not found'),
          409: error('Multiple plate matches require user selection'),
          ...bearer403,
        },
    ...
    '/api/v1/admin/payment/omise/qr': {
      get: {
        tags: ['Admin Payment Gateway'],
        summary: 'Proxy Admin Omise PromptPay QR image',
        description: 'Admin helper endpoint for displaying PromptPay QR images. Requires Bearer auth and transactions permission. Uses the saved Omise charge and does not apply mobile/kiosk/gate client source logic.',
        security: bearer,
        parameters: [
          query('chargeId', { type: 'string' }, 'chrg_test_123'),
          query('documentPath', { type: 'string' }, '/charges/chrg_test_123/documents/docu_test_123'),
    ...
    '/api/v1/client/check-in': {
      post: {
        ...
        description: 'Shared check-in endpoint for credentialed kiosk, barrier gate, camera, and printer devices. Use X-Device-Id and X-Device-Token. The backend detects the device type from the device record.',
        security: deviceAuth,
